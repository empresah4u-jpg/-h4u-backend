...
def semantic_search(
    ...
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            for entity_type, entity_id, content, similarity in semantic_rows:
                ...
                if entity_type == "hotel":
                    ...
                    if row:
                        results.append({
                            ...
                            "description": row[4],
                            "opening_hours": row[5],
                            "adult_price": float(row[6]) if row[6] is not None else None,
                            "child_price": float(row[7]) if row[7] is not None else None,
                            "currency": row[8],
                            "latitude": float(row[9]) if row[9] is not None else None,
                            "longitude": float(row[10]) if row[10] is not None else None,
                            "verification_status": row[11],
