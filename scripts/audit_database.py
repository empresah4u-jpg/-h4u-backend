...
CHECKS = {
    ...
    'paid_payment_cancelled_reservation': "SELECT count(*) FROM payments p JOIN reservations r ON r.id=p.reservation_id WHERE p.status='paid' AND r.status='cancelled' AND NOT EXISTS(SELECT 1 FROM cancellation_cases c WHERE c.reservation_id=r.id AND c.status='cancelled')",
