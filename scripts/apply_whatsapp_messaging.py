"""Explicit 007 runner: atomic, checksum-protected, no startup migrations."""
...
def apply(conn):
    ...
    for name in ('002_financial_adjustments.sql','003_identity_auth.sql','004_auth_security.sql','005_partner_memberships.sql','006_admin_audit.sql'):
    ...
    if row:
        if row!=(checksum,): raise RuntimeError('007 checksum mismatch')
        return False
...
def main():
    with get_connection() as conn:
        changed=apply(conn)
    print('007 applied atomically' if changed else '007 already applied; checksum verified')
